...
describe('4 - Implemente os casos de teste para a função `circle`', () => {
  it('Verifica se ao receber um raio, a função `circle` retorna um objeto contendo os valores esperados', () => {
    ...
    expect(circle(3).radius).toEqual(resultCircle.radius);
    expect(circle(3).area).toBeCloseTo(resultCircle.area);
    expect(circle(3).circumference).toBeCloseTo(resultCircle.circumference);
  });
});
