...
- Data limite para entrega: `01/09/2023 23:59`
...
1. Clone o repositório
   ...
   - `git clone git@github.com:tryber/sd-035-project-trybewarts.git`.
   - Entre na pasta do repositório que você acabou de clonar:
     - `cd sd-035-project-trybewarts`
...
4. Crie na raiz do projeto os arquivos que você precisará desenvolver:
   ...
   - Verifique se você está na raiz do projeto
     ...
     - Exemplo: `pwd` -> o retorno vai ser algo como _/Users/joao/code/**sd-035-project-trybewarts**_
...
7. Crie um novo `Pull Request` _(PR)_
   ...
   - Vá até a página de _Pull Requests_ do [repositório no GitHub](https://github.com/tryber/sd-035-project-trybewarts/pulls)
   ...
   - Volte até a [página de _Pull Requests_ do repositório](https://github.com/tryber/sd-035-project-trybewarts/pulls) e confira que o seu _Pull Request_ está criado
...
- Vá até a página do seu _Pull Request_, adicione a label de _“code-review”_ e marque pessoas da sua turma:
...
- Os requisitos do seu projeto são avaliados automaticamente, sendo utilizada a resolução de tela de `1366 x 768` (1366 pixels de largura por 768 pixels de altura). Recomenda-se desenvolver seu projeto usando a mesma resolução, via instalação [deste plugin](https://chrome.google.com/webstore/detail/window-resizer/kkelicaakdanhinjdeammmilcgefonfh?hl=en) do `Chrome` para facilitar a configuração da resolução;
...
Link: [Formulário de avaliação do projeto](https://be-trybe.typeform.com/to/ZTeR4IbH#cohort_hidden=CH35&template=betrybe/sd-0x-project-trybewarts)
...
<details>
  <summary>
    Os campos de entrada do tipo <code>checkbox</code> devem conter seis opções: <code>Comunicação, Trabalho em equipe, Negociação, Escuta ativa, Métodos de array, Jest</code> com a classe <code>subject</code> estando abaixo de uma label com <code>ID</code> igual a <code>label-content</code> cujo texto é <code>Qual conteúdo você está com mais vontade de aprender?</code>
  </summary> <br />
...
- Crie um input do tipo `checkbox` com a classe `subject` e o `value` igual a `Comunicação`;
- Crie um input do tipo `checkbox` com a classe `subject` e o `value` igual a `Trabalho em equipe`;
- Crie um input do tipo `checkbox` com a classe `subject` e o `value` igual a `Negociação`;
- Crie um input do tipo `checkbox` com a classe `subject` e o `value` igual a `Escuta ativa`;
- Crie um input do tipo `checkbox` com a classe `subject` e o `value` igual a `Métodos de array`;
- Crie um input do tipo `checkbox` com a classe `subject` e o `value` igual a `Jest`;
...
- O primeiro `input` do tipo `checkbox` deve possuir a classe `subject` e o atributo `value` igual a `Comunicação`;
- O segundo `input` do tipo `checkbox` deve possuir a classe `subject` e o atributo `value` igual a `Trabalho em equipe`;
- O terceiro `input` do tipo `checkbox` deve possuir a classe `subject` e o atributo `value` igual a `Negociação`;
- O quarto `input` do tipo `checkbox` deve possuir a classe `subject` e o atributo `value` igual a `Escuta ativa`;
- O quinto `input` do tipo `checkbox` deve possuir a classe `subject` e o atributo `value` igual a `Métodos de array`;
- O sexto `input` do tipo `checkbox` deve possuir a classe `subject` e o atributo `value` igual a `Jest`;
...
<details>
  <summary>
    Crie uma <code>textarea</code> com número máximo de caracteres igual à 500 abaixo de um <code>label</code> com o seguinte texto: <code>Descreva os principais pontos de aprendizado até aqui (1 hard skill / 1 soft skill):</code>
  </summary> <br />

- Crie uma `textarea`;
- Crie uma label com a classe `textarea` e que possua o texto **"Descreva os principais pontos de aprendizado até aqui (1 hard skill / 1 soft skill):"**;
- Adicione ao elemento `textarea` o limite máximo de 500 caracteres;
- Adicione ao elemento `textarea` o limite mínimo de 20 caracteres;
- Adicione ao elemento `textarea` o valor com o texto resposta à pergunta.

**O que será testado:**

- O elemento `label` deve possuir a classe `textarea` e o texto `Descreva os principais pontos de aprendizado até aqui (1 hard skill / 1 soft skill):`;
- O elemento `textarea` deve possuir um limite máximo de 500 caracteres;
- O elemento `textarea` deve possuir um limite mínimo de 20 caracteres;
- O elemento `textarea` deve incluir a resposta da pergunta, com um valor entre 20 e 500 caracteres.
